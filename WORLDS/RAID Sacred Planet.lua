...
-- sacred Planet 
local textLabel = game:GetService("Players").LocalPlayer.PlayerGui.LevelSelectGui.MapSelect.Main.Wrapper.Container.kai_planet.Main.Container.LevelsCleared.V
if textLabel and textLabel.Text then
    local levelNumber = tonumber(textLabel.Text:match("(%d+)")) -- Extracts first number and converts to number
    if levelNumber then
        levelNumber = math.min(levelNumber + 1, 5) -- Increment but do not exceed 5
        if levelNumber == 0 then levelNumber = 1 end -- Ensure 0 becomes 1
        local args = {
            [1] = "kai_planet_level_" .. levelNumber
        }
